...
PRIMARY_REMOTE_RPC_BASE = _normalize_remote_url("https://rpc.bdagscan.com")
...
def _apply_node_policies(ctx: "NodeContext", settings: Dict[str, bool]) -> None:
    ...
    if failsafe:
        ...
        if "worker stopped" in str(failsafe_reason or "").lower():
            # Treat worker exits as offline, not stalled
            metrics["running"] = False
            metrics["container_running"] = False
            metrics["stalled"] = False
            metrics["stalled_reason"] = "worker stopped"
...
# API endpoints
# ---------------------------------------------------------------------------
APP_VERSION = os.getenv("BDAG_MANAGER_VERSION", "v1.7.2").strip() or "v1.7.2"

...
```bash
curl -fsSL https://raw.githubusercontent.com/murat-taskaynatan/Blockdag-Node-Manager/v1.5.6/install_nm_from_github.sh \ | sudo bash
```

The installer accepts `REPO_REF`/`REPO_BRANCH`/`REPO_URL` environment variables if you want to pin to a different tag or fork. Running from a tagged path (like `v1.5.6` above) guarantees you pull the matching release assets even while `main` is still in progress.

...
def _prepare_ports(config: Dict) -> Tuple[int, int]:
    ...
    if config.get("autoPorts"):
        ...
        if base_p2p in used or base_rpc in used:
...
def _render_compose(source: Path, target: Path, label: str, p2p: int, rpc: int):
    ...
    text = text.replace('- "18546:18546"', f'- "{ws_port}:{ws_port}"', 1)
    text = text.replace("--rpclisten=0.0.0.0:38131", f"--rpclisten=0.0.0.0:{p2p}")
    text = text.replace("--http.port=18545", f"--http.port={rpc}")
    text = text.replace("--ws.port=18546", f"--ws.port={ws_port}")
    text = text.replace("ws://127.0.0.1:18546", f"ws://127.0.0.1:{ws_port}")
...
def launch_node(payload: Dict) -> Dict:
    ...
    p2p_port, rpc_port = _prepare_ports(payload)
    ...
    _render_compose(compose_src, compose_target, label, p2p_port, rpc_port)
